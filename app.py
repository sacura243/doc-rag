"""Streamlit UI for the enterprise knowledge base assistant."""
...
from doc_rag.rag import ask_with_sources, ingest_files, retrieve_sources
...
EVALUATION_CASES = [
    {"question": "新用户注册后可以使用哪些核心功能？", "expected": "产品功能说明.txt"},
    {"question": "退款申请多久完成首次回复？", "expected": "客服退款SOP.txt"},
    {"question": "涉及重复扣费时应如何处理？", "expected": "客服退款SOP.txt"},
    {"question": "产品经理本周需要完成什么工作？", "expected": "产品周会纪要.txt"},
    {"question": "测试同学如何验证知识库问答效果？", "expected": "产品周会纪要.txt"},
]
...
def apply_style() -> None:
    st.markdown(
        """
        <style>
        :root { --ink:#192534; --muted:#657789; --line:#d8e1e8; --accent:#167c87; }
        ...
        .stButton > button { background:var(--accent); color:#fff; border:1px solid var(--accent); border-radius:6px; min-height:2.35rem; font-weight:600; }
...
with st.sidebar:
    st.markdown("### 知识库管理")
    st.caption("导入产品文档、运营 SOP、客服 FAQ、会议纪要或任意业务资料。")
    ...
    if source_items:
        for item in source_items:
            ...
            with delete_col:
                if st.button("删", key=f"delete_{item['source']}", help=f"删除 {display_source_name(item['source'])}"):
                    deleted = store.delete_source(item["source"])
...
st.title("企业知识库智能助手")
st.markdown("面向产品、运营、客服等团队的本地知识库问答。每条回答均可回看原文依据。")
st.markdown(
...
chat_tab, evaluation_tab = st.tabs(["智能问答", "检索评测"])
...
with chat_tab:
    ...
    selected = st.selectbox("业务场景示例", sample_questions, label_visibility="collapsed")
...
with evaluation_tab:
    st.subheader("检索效果评测")
    st.caption("评测不调用大模型，只验证每个问题是否检索到预期业务资料，用于检查知识库与检索策略。")
    if st.button("运行 5 条演示评测", disabled=chunk_count == 0):
        results = []
        progress = st.progress(0)
        for index, case in enumerate(EVALUATION_CASES, start=1):
            sources = retrieve_sources(cfg, case["question"], top_k=3)
            names = [display_source_name(item["source"]) for item in sources]
            results.append(
                {
                    "问题": case["question"],
                    "预期资料": case["expected"],
                    "Top-3 命中资料": "、".join(names) or "无",
                    "通过": "是" if case["expected"] in names else "否",
                }
            )
            progress.progress(index / len(EVALUATION_CASES))
        st.session_state.evaluation_results = results

    if "evaluation_results" in st.session_state:
        results = st.session_state.evaluation_results
        passed = sum(item["通过"] == "是" for item in results)
        st.metric("Top-3 来源命中率", f"{passed / len(results):.0%}", f"{passed}/{len(results)} 条命中")
        st.dataframe(results, use_container_width=True, hide_index=True)
    elif chunk_count == 0:
        st.info("请先导入互联网产品资料样例或上传自己的业务资料。")
